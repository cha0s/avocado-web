...
Window = module.exports = class
	...
	setUpKeyEventHandlers: ->
		...
		@Canvas.onkeydown = (event) =>
			...
			@keyDowns_.push code: keyCodeMap (event ? window.event).keyCode
			
		document.onkeyup = (event) =>
			
			@keyUps_.push code: keyCodeMap (event ? window.event).keyCode
	...
	# Even though the browser gives us a real async environment for events, I
	# will keep them in a dispatch loop until maybe down the line it can be
	# proven that it isn't required for consistency.
	'%pollEvents': ->
